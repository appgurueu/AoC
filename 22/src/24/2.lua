...
local function mins_needed(sx, sy, tx, ty)
	for _, row in pairs(grid) do
		for _, cell in pairs(row) do
			if cell ~= wall then
...
local to_goal = mins_needed(sx, sy, tx, ty)
local back = mins_needed(tx, ty, sx, sy)
local back_to_goal = mins_needed(sx, sy, tx, ty)

return to_goal + back + back_to_goal
