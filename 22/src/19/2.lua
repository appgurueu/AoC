...
local function prune_prospective_geodes(states, blueprint, mins_left)
	local max_min_prospective_geodes = -math.huge
	local top_state
	for _, state in ipairs(states) do
		local min_prospective_geodes = state[geode] + state[robot + geode] * mins_left
		if min_prospective_geodes > max_min_prospective_geodes then
			top_state = state
			max_min_prospective_geodes = min_prospective_geodes
		end
	end
	local pruned = {}
	for _, state in ipairs(states) do
		...
		if state == top_state or max_prospective_geodes > max_min_prospective_geodes then
...
local function calc_max_geodes(blueprint)
	...
	for min = 1, 31 do
		...
		states = prune_states(new_states, blueprint, 32 - min)
	end
	-- Last minute: There is no point in building robots
	for _, state in ipairs(states) do
		for j = ore, geode do
			state[j] = state[j] + state[j + robot]
		end
...
local prod = 1
for id = 1, 3 do
	local max_geodes = calc_max_geodes(blueprints[id])
